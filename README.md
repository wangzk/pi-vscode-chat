...
- **Completion notifications** — a native VS Code notification when the agent finishes, so you can context-switch away while it works. Skipped when you're already watching the chat (override with `piChat.notifyAlways`). Optionally also fire an OS-level desktop notification (`piChat.notifyDesktop`) visible outside VS Code. Configure via `piChat.notifyOnComplete`.
