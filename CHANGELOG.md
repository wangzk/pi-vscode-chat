...
- **Multiple carets blinking simultaneously during long multi-turn runs** (the most visible symptom): each time a tool call split the stream into a new text block, the previous block kept its `live` caret until `agentEnd` — a text → tool → text → … run showed every block blinking at once (3–4 carets in real sessions). Only the newest block is live now.
- **Empty text blocks after tool calls**: the render throttle used a single global pending slot, so a new block's delta could overwrite an earlier block's final render, leaving it permanently empty. Pending renders are now keyed per container.
